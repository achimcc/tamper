...
fn lotse_exit_200_is_a_queue_timeout_and_201_is_the_network() {
    ...
    assert!(matches!(classify(201, ""), Outcome::Network));
}
...
fn a_network_failure_that_reaches_us_anyway_is_not_a_ruling() {
    ...
    let out = "error: unable to download 'https://cache.nixos.org/…': Could not resolve host";
    assert!(matches!(classify(1, out), Outcome::Network));
}
